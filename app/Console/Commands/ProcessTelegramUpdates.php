...
{
    protected $signature = 'telegram:process';
    protected $description = 'Process Telegram updates manually';
    ...
    public function handle(TelegramService $telegram)
    {
        ...
        if (!$token) {
            $this->error('TELEGRAM_BOT_TOKEN missing');
            return 1;
        }

        $response = Http::get("https://api.telegram.org/bot{$token}/getUpdates");

        if (!$response->successful() || !$response['ok'] || empty($response['result'])) {
            return 0;
        }
    ...
    private function handleCallback(array $callback, string $token)
    {
        $data = $callback['data'];
    ...
    private function handleMessage(array $message, TelegramService $telegram)
    {
        ...
        match (true) {
            $text === '/start' => $this->sendWelcomeMessage($telegram, $chatId, $firstName),
            $text === '/estado' => $this->checkStatus($telegram, $chatId),
            $text === '/notas' => $this->listNotes($telegram, $chatId),
